...
const unsigned long eventIntervalUpdateSensor = 1 * 20 * 1000;
...
void MqttCallback(String topic, byte* payload, unsigned int _length) {
  ...
  String prefix = "@msg/device/";
  if (topic.startsWith(prefix)) {
    int lastSlashIndex = topic.lastIndexOf('/');
    if (lastSlashIndex != -1) {
      String _command = topic.substring(lastSlashIndex + 1);
      String deviceId = topic.substring(prefix.length(), lastSlashIndex);
      if (deviceId == MqttClientId) {
        if (_command == "start") {
          Serial.println("Received START command for our device!");
          isRunningDevice = true;
          preferences.putBool("isRunningDevice", true);
          playStartMelody();
        }
        else if (_command == "stop") {
          Serial.println("Received STOP command for our device!");
          isRunningDevice = false;
          preferences.putBool("isRunningDevice", false);
          playStopMelody();
        }
          ...
          if (file) {
            ...
            formulaReceived();
            hasFormula = true;
            preferences.putBool("hasFormula", true);
            Serial.println("JSON saved to SD Card!");
          } else {
            Serial.println("Failed to open file for writing");
          }
        }
        else if (_command == "firmware") {
...
/* -------------------- Formula State ------------------------ */
/* Main function: Read data from SD Card and call subfunctions */
void readPeriodAndFormula() {
  ...
  SD.end();
  Serial.print("use JSON size: ");
  Serial.println(measureJson(recipe));
}

/* read period (startDate, endDate) */
...
/* Read formula (calls readSchedulesData() & readValuesData()) */
...
void readSchedulesData(JsonObject stage, FormulaState &state) {
  if (stage.containsKey("schedules")) {
    ...
    for (JsonObject schedule : schedules) {
      ...
      // Convert executionTime into startTime & endTime (minutes)
  ...
  for (int i = 0; i < formulaCount; i++) {
    ...
    if (formulaStates[i].scheduleCount > 0) {
      ...
      for (int j = 0; j < formulaStates[i].scheduleCount; j++) {
        ...
        Serial.println(s.repeatOnBitmask, BIN);
      }
    }
...
int calculateCurrentDay() {
  ...
  // Calculate how many days have passed from the start date to the current date
  int dayDifference = (now.unixtime() - periodStart.unixtime()) / 86400;
  int currentDay = dayDifference + 1; // start day 1
  ...
  // Check if it exceeds the period
...
void sendValueDataToSlave(int currentStateIndex) {
  ...
  uint16_t valueData[MAX_VALUES * 3];  // Data to be sent
  ...
  for (int j = 0; j < 2; j++) { // valueCount
    ...
    valueData[j] = v.targetStart;  // Multiply by 100 to convert to int
...
void sendScheduleDataToSlave(int currentStateIndex) {
  ...
  uint16_t scheduleData[MAX_SCHEDULES * 3];  // Data to be sent
...
void updateRTCFromInternet() {
  ...
  int year    = timeClient.getEpochTime() / 31556926 + 1970;  // Convert UNIX Time to year
...
void hasFormulaCheck() {
  if (SD.begin(SD_CS)) {
    if (SD.exists("/data.json")) {
      hasFormula = true;
      preferences.putBool("hasFormula", true);
      Serial.println("Found formula file on SD card");
    } else {
      hasFormula = false;
      preferences.putBool("hasFormula", false);
    }
    SD.end();
  }
}
...
void setup() {
  ...
  isRunningDevice = preferences.getBool("isRunningDevice", true);
  hasFormula      = preferences.getBool("hasFormula", false);
  //hasFormulaCheck();
  Serial.println();
  Serial.print("Initial state - Running: ");
  Serial.print(isRunningDevice ? "Yes" : "No");
  Serial.print(", Has Formula: ");
  Serial.println(hasFormula ? "Yes" : "No");
...
void loop() {
  ...
  if (isRunningDevice && hasFormula) {
    ...
    if (millis() - lastNotifyTime > 60000) {
      Serial.println("Waiting for planting formula...");
      playNoFormulaMelody();
  ...
  for (;;) {
    ...
    if (WiFi.status() == WL_CONNECTED && !client.connected()) {
      ...
      if (client.connect(MqttClientId.c_str(), MqttToken.c_str(), MqttSecret.c_str())) {
        ...
        String subtopic = "@msg/device/";
        subtopic       += MqttClientId;
        subtopic       += "/#";
        ...
        /*------------------------------------*/
        // Read isRunningDevice Status command
      } else {
...
void playStartMelody() {
  tone(BUZZER_PIN, 262, 150); delay(200);
  tone(BUZZER_PIN, 330, 150); delay(200);
  tone(BUZZER_PIN, 392, 150); delay(200);
  tone(BUZZER_PIN, 523, 300); delay(350);
  noTone(BUZZER_PIN);
}
...
void playNoFormulaMelody() {
  tone(BUZZER_PIN, 392, 100); delay(150);
  tone(BUZZER_PIN, 330, 100); delay(150);
  tone(BUZZER_PIN, 392, 100); delay(150);
  tone(BUZZER_PIN, 330, 100); delay(150);
  noTone(BUZZER_PIN);
}
...
void resetSystemState() {
  isRunningDevice = true;
  hasFormula      = false;
  preferences.putBool("isRunningDevice", isRunningDevice);
  preferences.putBool("hasFormula", hasFormula);
